...
When ~fzf-native~ is the active backend (~fussy-score-fn~ set to
~fussy-fzf-native-score~ or via ~fussy-setup-fzf~), the C-side
...
For an asynchronous, shell-streaming alternative that sidesteps
~fussy~ entirely and talks to ~fzf-native~ directly, see
[[https://github.com/jojojames/fzf-async/blob/HEAD/architecture.org][fzf-async]].
