...
non-nil (the new default) — fzf-native runs its cheap ~fzf_has_match~
path instead of full DP scoring, and ~fussy~ stays in the completion
pipeline so its candidate pool subsumes across keystrokes.  At 95k
candidates the absolute win over full scoring is modest (~6–10%); it
widens with pool size because the per-candidate inner loop is ~5×
cheaper.
...
|-------------------------------------+---------+--------------------------------------------------------------------------|
| ~fussy-company-prefix-length~       | 3       | Bypass cutoff: prefixes strictly shorter than N strip fussy from ~completion-styles~ and run plain completion. |
| ~fussy-company-filter-only-length~  | 3       | Filter-only cutoff: prefixes strictly shorter than N keep fussy in the pipeline but take fzf-native's cheap ~fzf_has_match~ path.  Overrides the bypass when both apply. Set to ~nil~ to use the bypass instead.  Only meaningful when ~fussy-score-ALL-fn~ is ~fussy-fzf-score~. |

By default short prefixes go through filter-only (`fzf` route) — ~fzf-native~
does the cheap filtering, fussy stays in ~completion-styles~ so its candidate
pool subsumes naturally across keystrokes, and the eventual
full-scoring keystroke (prefix length ≥ N) scans an already-narrow
set.

If you prefer the older bypass behavior — short prefixes skip fussy
entirely, falling back to plain ~completion-styles~ — set:
#+begin_src emacs-lisp
(setq fussy-company-filter-only-length nil)
#+end_src
...
[[https://github.com/jojojames/fzf-async][fzf-async]] is the companion sibling project to ~fussy~. Where ~fussy~
...
~fzf-async~ takes a different approach: it streams candidates from
...
~fzf-async~ when you want to stream results from an external tool like
...
- [[https://github.com/jojojames/fzf-async/blob/HEAD/architecture.org][fzf-async/architecture.org]] — the companion sibling project.
  ...
  and why ~fzf-async~ deliberately routes around ~fussy~ via its own
  completion category.
